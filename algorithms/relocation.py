...
class DynamicRelocation:
    ...
    def _undo_suggestion(self, suggestion: dict) -> None:
        """Relocation önerisini geri al."""
        asc = suggestion['asc_item']
        new_loc = asc.location
        old_loc_asc = suggestion['asc_item'].location  # güncel lokasyon

        # Ascending geri al
        prev_loc = [k for k, v in self.loc_to_item.items() if v == asc.item_id]
        if prev_loc:
            cur = prev_loc[0]
            self.loc_to_item.pop(cur, None)
        orig_loc = suggestion.get('_orig_asc_loc', new_loc)

        # Basitleştirilmiş undo: lokasyonları sıfırla
        asc.location = suggestion.get('_orig_asc_loc', asc.location)
        asc.current_class = suggestion.get('_orig_asc_cls', asc.current_class)
        self.loc_to_item[asc.location] = asc.item_id

        desc = suggestion.get('desc_item')
        if desc:
            desc.location = suggestion.get('_orig_desc_loc', desc.location)
            desc.current_class = suggestion.get('_orig_desc_cls', desc.current_class)
            self.loc_to_item[desc.location] = desc.item_id
    ...
    def _compute_td(self, orders: list[Order], algorithm) -> float:
        ...
        Hız için: NN routing (2-opt olmadan) kullanılır.
        Paper: DEPSO ile hesaplar ama bu çok yavaş — NN yeterince yakın.
        """
        ...
        total = 0.0
        for b in batches:
            _, dist = nearest_neighbor_route(b.locations, self.wh)
            total += dist
        return total
