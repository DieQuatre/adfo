...
class RBRS_AE(BatchingRoutingAlgorithm):
    ...
    def _regret_assignment(self, orders: list[Order],
                           priorities: list[float]) -> list[Batch]:
        """
    ...
    def _eliminate(self, batches: list[Batch], all_orders: list[Order],
        ...
        scored = sorted(
            batches,
            key=lambda b: (0.7 * b.travel_distance / max(b.num_orderlines, 1) +
        ...
        # Regret ile yeniden ata, önce kept batch'lere sığdır
        freed_batches = self._regret_assignment(freed, freed_pri)
        for fb in freed_batches:
            for o in fb.orders:
                placed = False
                for b in kept:
                    if b.total_weight + o.total_weight <= self.capacity:
                        b.orders.append(o)
                        b.total_weight += o.total_weight
                        placed = True
                        break
                if not placed:
                    kept.append(Batch(batch_id=len(kept), orders=[o],
                                      total_weight=o.total_weight))
    ...
    def _savings_start(self, orders: list[Order]) -> list[Batch]:
        """
        Clarke-Wright savings ile hızlı başlangıç batch'leri üret.
        DEPSO'nun savings particle'ı ile aynı mantık.
        """
